        ...
        try {
            ...
            while (!battleUdpTunnelCancellationToken.IsCancellationRequested) {
                ...
                if (null != batch && 0 < batch.Count) {
                    ...
                    /*
                    [WARNING] Different from frontend concerns, it's actually safe to update "ifd.ConfirmedList" (where "ifd" belongs to the "inputsBuffer") by an UDP inputFrameUpsync, as long as all updates to "ifd.ConfirmedList" and "room.lastAllConfirmedInputFrameId" are guarded by "inputsBufferLock" -- hence in Golang version, both "markConfirmationIfApplicable" and "forceConfirmationIfApplicable" are guarded by "inputsBuffer".
                    */
                }
            }
